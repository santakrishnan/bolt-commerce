  ...
  return (
    <>
      ...
      <div className={cn("relative w-full lg:w-131", showSuggestions ? "z-50" : "")}>
        <div
          className={cn("bg-white shadow-sm", isSearchOpen ? "rounded-t-[1.5rem]" : "rounded-full")}
        >
          <div
            className={cn(
          ...
          >
            ...
            <div className="relative w-full flex-1">
              {!(searchQuery || showSuggestions) && (
                <div className="pointer-events-none absolute inset-0 flex items-center font-[var(--font-family,'Toyota_Type')] text-[var(--font-size-sm)] leading-[125%] tracking-[-0.14px]">
                  <span className="text-[var(--Core-surfaces-foreground)]/60">
                    SUV under 35k with&nbsp;
                  </span>
                  <span className="font-semibold text-[var(--Core-surfaces-foreground)]">
                    low miles...
                  </span>
                </div>
              )}
              <input
                className={cn(
                  ...
                  "text-[var(--Core-surfaces-foreground)]",
            ...
            />
            <button
              className="hidden cursor-pointer rounded-full bg-red-500 px-[var(--spacing-lg)] py-[var(--spacing-xs)] font-medium text-white transition-colors hover:bg-red-600 lg:block"
              onClick={() => {
                closeSuggestions();
                handleSearch();
              }}
        ...
        {showSuggestions && (
          <div
            className={cn(
              ...
              "rounded-b-[2rem] border border-gray-300 border-t-0 bg-white shadow-lg",
            ...
            {searchSuggestions.map((suggestion, idx) => (
              <button
                ...
                onClick={() => {
                  ...
                  closeSuggestions();
                  setTimeout(() => {
                    navigate(`${suggestion.text} ${suggestion.highlight}`);
                  }, 250);
                }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded bg-white p-[var(--spacing-lg)] text-center">
            <p className="mb-[var(--spacing-md)] text-[length:var(--font-size-sm)]">
              This microphone is still work in progress. Redirecting to Vehicle listing Page?
            </p>
